...
export default function RegulasiCard({
  ...
  if (view === "list") {
    return (
      <Link
        ...
        className="status-tab group flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-5 surface-card hover:shadow-card-hover hover:border-accent/30 hover-lift p-4 sm:pl-6"
  ...
  return (
    <Link
      href={`/regulasi/${regulasi.id}`}
      className="status-tab group block surface-card hover:shadow-card-hover hover:border-accent/30 hover-lift p-5"
      style={{ "--tab-color": TAB_COLOR[regulasi.status] } as React.CSSProperties}
    >
